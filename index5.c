...
/* Разбить в float на части, получить знак, порядок и мантиссу.

3 пути ведут к одному. Получить биты которые отвечают за знак, порядок и мантису.

1 путь. С помощью указателей. Делаем так, чтобы указатель указывал на участок памяти где хранится float как на целое число.
Далее достаем биты: Старший бит - знак. следующие 8 бит - порядок, оставшиеся это мантисса.

2 путь. Union. Суть в том что переменные int и float занимают в памяти одно и то же место. Запиcываем значение в float.
Потом обращаемся к числу int и рассматриваем его биты, как в предыдущем примере.

3 Путь. Битовые поля. 
Объявляя битовые поля указываем, сколько бит будет выделять под мантиссу, порядок и знак. 
23, 8, 1 бит соответственно.
Битовые поля задавать нужно именно в этом порядке.
Ну и потом обращаемся к этим полям. В них уже хранятся нужные нам значения.

Далее для всех одинаково. Мы достали биты для знака, поорядка и мантиссы. Осталось сделать вывод. */
